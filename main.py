...
async def file_echo(message: types.Message):
    ...
    await message.document.download(destination_file='./download.csv')
    await message.answer('⏳ Wait a few seconds')
    csv_to_xlsx('./download.csv')
    await message.reply_document(open('./results.xlsx', 'rb'), caption='File is ready!')
...
@dp.message_handler(commands=['cyrillic2latin'], content_types=['text'])
# @auth
async def cyrillic2latin_handler(message: types.Message):
    user_message = message.text

    if user_message.isascii():
        reply = to_cyrillic(user_message)
    else:
        reply = to_latin(user_message)
...
async def qr_handler(message: types.Message):
    ...
    await message.answer_photo(bio, caption=f"QR code of {user_text}")


@dp.message_handler(commands=['getmyid'], content_types=['text'])
async def handle_get_id(message: types.Message):
    ...
    if message.text == '/getmyid':
        ...
        await message.answer(f'🙅🏻 ‍Я не понимаю {message.content_type}')
